...
class CoinbaseExchangeAuth(AuthBase):
    ...
    def __call__(self, request):
        ...
        message = timestamp + request.method + request.path_url + (request.body or b"").decode("utf-8")
...
class CoinbaseAPI:
    ...
    def placeOrder(self,size,price,side,product):
        r = requests.post(self.api_url+"orders",json={
                                                    "size" : size,
                                                    "price" : price,
                                                    "side" : side,
                                                    "product_id" : product,
                                                    "stp" : "cb",
                                                    "time_in_force":"GTT",
                                                    "cancel_after":"min"},auth=self.auth)
        return r.json()
    ...
    def getOrderBook(self,product,level=2):
        r = requests.get(self.api_url+"products/{}/book?level={}".format(product,level),auth = self.auth)
        return r.json()

    def getHistoric(self,product,granularity = 86400,start = None,end=None):
        if not start or not end:
            r = requests.get(self.api_url+"products/{}/candles?granularity={}".format(product,granularity),auth = self.auth)
        else:
            r =  requests.get(self.api_url+"products/{}/candles?granularity={}&start={}&end={}".format(product,granularity,start,end),auth = self.auth)
     
        return r.json()

    def getBaseQuote(self,product):
        r = requests.get(self.api_url+"products/{}".format(product),auth = self.auth)
        return r.json()["base_increment"].count("0"), r.json()["quote_increment"].count("0")
    ...
    def getTrades(self,product):
        r = requests.get(self.api_url+"products/{}/trades".format(product),auth = self.auth)
        return r.json()
    ...
    def getFills(self, product,before):
        r = requests.get(self.api_url+"products/{}/trades?before={}".format(product,before))
        #print(r.headers)
    ...
    def getProductNames(self):
        ...
        return [item["id"] for item in r.json() if not (item["trading_disabled"] and item["cancel_only"] and item["post_only"] and item["limit_only"])]
