...
class Agent:
    ...
    def updated_critic(self, td_estimate, qf1, qf2):
        ...
        nn.utils.clip_grad_norm_(
            [p for g in self.critic_swa.param_groups for p in g["params"]],
            self.max_grad_norm,
        )  # gradient clipping
    ...
    def update_actor(self, log_probs, min_qf_pi):
        ...
        loss.backward()
        nn.utils.clip_grad_norm_(
            [p for g in self.actor_swa.param_groups for p in g["params"]],
            self.max_grad_norm,
        )
        self.actor_swa.step()
        return loss.item()
    ...
    def update_alpha(self, log_probs):
        ...
        loss.backward()
        nn.utils.clip_grad_norm_(
            [p for g in self.alpha_optim.param_groups for p in g["params"]],
            self.max_grad_norm,
        )
    ...
    def learn(self):
        ...
        for _ in range(self.num_epochs):
            ...
            td_target = self.td_target(
                reward, next_state_action, next_state, next_state_log_pi, done
            )
...
class ActorNet(nn.Module):
    ...
    def __init__(
        self, state_dim, action_dim, activation=nn.LeakyReLU, hidden_layers=64
    ):
        super().__init__()
    ...
    def forward(self, X):
        ...
        log_std = torch.clamp(
            log_std, min=ActorNet.LOG_SIG_MIN, max=ActorNet.LOG_SIG_MAX
        )
        return mean, log_std
...
class CriticNet(nn.Module):
    ...
    def forward(self, state, action):
        ...
        X = torch.cat(
            [
                state.reshape(-1, self.features).float(),
                action.reshape(-1, self.actions).float(),
            ],
            1,
        )
...
class DeterministicActorNet(nn.Module):
    ...
    def sample(self, state):
        ...
        noise.clamp(-0.25, 0.25)
        action = mean + noise
        return (
            action,
            torch.zeros(action.shape[0], action.shape[1], device=self.device).unsqueeze(
                1
            ),
            mean,
        )
