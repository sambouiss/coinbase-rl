...
class Enviroment:
    ...
    def __init__(self, start_time, end_time, api, products, session_length = 3600, penalty = .005):
    ...
    def setup_env(self):
        ...
        makerFee, takerFee = float(fees["maker_fee_rate"]), float(fees["taker_fee_rate"])
        ...
        for item in np.random.permutation(len(self.products)):
            ...
            productFeatures = ProductFeatures(product,api,makerFee,takerFee,self.startTime,sessionLength=self.session_length)
            
            self.product_features[product] = productFeatures
        
            sellPos = productFeatures.sellBalance*(productFeatures.midPoint*productFeatures.botPrice)
    ...
    def stepEnv(self):
        ...
        timeDelta =  (newUpdateTime-self.lastUpdateTime).total_seconds()/self.session_length
        state = [ []  for _ in range(len(self.products))]
        ...
        for item in np.random.permutation(len(self.products)):
            ...
            new_state,posPnl,can_update = productFeatures.update(makerFee, takerFee, newUpdateTime, api)
            flag = flag or can_update
            self.product_features[product] = productFeatures
            self.pos_pnl=posPnl*self.gamma+(1-self.gamma)*self.pos_pnl
            newTotal+=productFeatures.sellBalance*productFeatures.midPoint*productFeatures.botPrice
    ...
    def expertAction(self):
        ...
        for product in self.products:
            productFeatures = self.product_features[product] 
            midPoint = productFeatures.midPoint
            theo = productFeatures.avgPrice
            var = productFeatures.ewmaVar
            buyBalnce = productFeatures.buyBalance*midPoint
            sellBalance = productFeatures.sellBalance
            base = productFeatures.base
            buySize = productFeatures.buySize*midPoint
            sellSize = productFeatures.sellSize
            buySize = round(min(buySize,.1*(sellBalance+buyBalnce)), base)
            sellSize = round(min(sellSize,.1*(sellBalance+buyBalnce)), base)
            lamb = productFeatures.lamb
            timeRemaining = productFeatures.timeRemaining
            sessionLength = productFeatures.sessionLength
            netPosition = (sellBalance-buyBalnce)/(buyBalnce+sellBalance)
            t = (1-timeRemaining/sessionLength)
            var*=t
            spread = productFeatures.spread
            quoteInc = productFeatures.quoteInc
            quote = productFeatures.quote
            bestBid = midPoint-spread*quoteInc/2
            bestBid = round(bestBid,quote)
            bestAsk = midPoint+spread*quoteInc/2
            bestAsk = round(bestAsk,quote)
            reservationPrice = theo-var*netPosition*self.gamma
            offset = self.gamma*var*netPosition+2*np.log(1+lamb*self.gamma/(sessionLength))/self.gamma*midPoint 
            makerFee = productFeatures.makerFee
            takerFee = productFeatures.takerFee
            
            bid = reservationPrice-offset/2
            ask = reservationPrice+offset/2
           
            if bestBid-reservationPrice>takerFee*midPoint*2:
                action = [(spread+1),-(spread+1)]
            elif reservationPrice-bestAsk>takerFee*midPoint*2:
                action = [-(spread+1),(spread+1)]
            elif len(productFeatures.orders)==2:
                action = [-2*spread,-2*spread]
            elif ask-bid<2*makerFee*midPoint or (ask-bid)/quoteInc>10:
                offset = (2*makerFee*midPoint)/quoteInc-spread/2
                action = [offset,offset]
            else:
                bidOffset = (bestBid - bid)/quoteInc
                askOffset = (ask - bestAsk)/quoteInc
                action = [bidOffset,askOffset]
            productFeatures.act(action)
...
class ProductFeatures:
    ...
    def __init__(self, product, api, makerFee, takerFee, startTime, gamma = .9, sessionLength=3600):
        self.gamma =gamma
        self.rho = .3
        self.api = None
        ...
        self.vwop_bid = 0
        self.vwop_ask = 0
        self.starting_cash = .5
        self.starting_stock = .5
    ...
    def update(self, makerFee, takerFee, updateTime, api):
        ...
        if self.midPoint is None:
            self.startingPoint=oldMidPoint =  midPoint = self.midPoint = (bestAsk+bestBid)/2
        else:
        ...
        self.ewmaVar = self.rho*(midPoint-self.avgPrice)**2+(1-self.rho)*self.ewmaVar

        bidVol, askVol = sum(float(bids[i][1]) for i in range(len(bids))), sum(float(asks[i][1]) for i in range(len(asks)))
        ...
        for _ in range(len(self.orders)):
            ...
            if "status" in r and r["status"] == "done" and r["done_reason"]== "filled":
                fillTime = datetime.datetime.utcnow()-datetime.datetime.fromisoformat(r["created_at"].split(".")[0])
                ...
                if r["side"] == "buy":
                    ...
                    self.avgPrice = (volumeTop*(price) + self.avgPrice*self.tradingVolume)/(volumeTop+self.tradingVolume)
                ...
                if fillTime>600:
                    ...
                    if r["side"] == "buy":
                        ...
                        self.vwop_bid+=(midPoint-quoted_val)/self.quoteInc*quoted_size
                        buy_vol+=quoted_size
                    if r["side"] == "sell":
                        self.vwop_ask+=(quoted_val-midPoint)/self.quoteInc*quoted_size
    ...
    def getProductFeatures(self):
        ...
        ord_dis = (bid_dispersion-ask_dispersion)/(normalizedBuyBal+normalizedSellBal)
        out = [
                100*100*(midPrice-self.avgPrice)/self.startingPoint,
                10*(normalizedBuyBal-normalizedSellBal)/(normalizedBuyBal+normalizedSellBal),
                ord_dis,
                ...
                10*(normalizedAskVol-normalizedBidVol)/(normalizedBidVol+normalizedAskVol)
                ]
        # self.state_scaler.partial_fit([out])
        # return self.state_scaler.transform([out])[0].astype("float32")
        return out
    ...
    def act(self, action):
        ...
        buySize = min(.05*(self.buyBalance/self.midPoint+self.sellBalance),.1*self.buySize/self.midPoint)
        buySize = round(buySize,self.base)
        sellSize = min(.05*(self.buyBalance/self.midPoint+self.sellBalance),.1*self.sellSize)
